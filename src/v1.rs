...
pub fn flash_attention_v1(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if let Some(kernel) = Avx512Kernel::new() {
            run_v1(&kernel, q, k, v, seq_len_q, seq_len_k, d_head, config, out);
            return;
        }
    ...
    out.par_chunks_mut(br * d_head).enumerate().for_each_init(
        ...
        |(m_buf, l_buf, acc_buf, scores_buf), (qi, out_block)| {
            ...
            for kj in 0..num_kv_blocks {
                ...
                // S_ij = scale * Q_i . K_j. Three-tier register blocking
                // — see `v2.rs`'s identical QK^T loop / `Kernel::dot4x4`'s
                // docs for why.
                let mut i = 0;
                while i + 4 <= this_br {
                ...
                while i + 4 <= this_br {
                    ...
                    for r in 0..4 {
                        ...
                        kernel.scale_inplace(acc_row, old_l * correction);
                ...
                // `v2.rs`'s identical PV loop / `Kernel::pv4`'s docs for why.
                let mut i = 0;
                while i + 4 <= this_br {
                ...
                for i in 0..this_br {
                    ...
                    let acc_row = &mut acc[i * d_head..(i + 1) * d_head];
                    kernel.scale_inplace(acc_row, inv_new_l);
                }
            }
...
mod tests {
    ...
    fn check_kernel<K: Kernel + Sync>(
        ...
        run_v1(kernel, &q, &k, &v, seq_q, seq_k, d, &config, &mut out);
    ...
    fn fully_future_tile_is_a_noop() {
        check_kernel(&ScalarKernel::new(), 4, 64, 16, true);
    }
}
